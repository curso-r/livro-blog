...
![](assets/imgs/monty-hall/diag-ex.png)
...
```{r modelo-monty-hall, eval=FALSE}
...
simular_monty_hall <- function(z = 0) {
  ...
  if (trocar == "s") {
    escolhas <- c(escolha_inicial, porta_retirada)
    escolha_final <- setdiff(v, escolhas)
  } else {
    escolha_final <- escolha_inicial
  }
  result <- ifelse(
    escolha_final == ferrari, 
    "ganhei", 
    "perdi"
  )
  
  # guardando no BD
  tibble::tibble(escolha_inicial, ferrari, 
                 porta_retirada, trocar, result)
}

dados_simulados <- seq_len(1e4) %>% 
  purrr::map_dfr(simular_monty_hall) %>% 
  dplyr::mutate_all(as.factor)
