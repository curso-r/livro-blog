...
area <- read_excel('area.xls') %>%
  clean_names() %>%
  filter(!is.na(id)) %>% 
  dplyr::select(uf = nm_uf_sigla, cd_gcmun, muni = nm_mun_2016, area = ar_mun_2016)
```

```{r, message=FALSE, warning=FALSE, echo = FALSE}
area <- read_excel('assets/data/sao-paulo/area.xls') %>%
  clean_names() %>%
  filter(!is.na(id)) %>% 
  dplyr::select(uf = nm_uf_sigla, cd_gcmun, muni = nm_mun_2016, area = ar_mun_2016)
```
...
```{r message=FALSE, warning=FALSE, echo = FALSE}
loc <- locale(grouping_mark = '.', decimal_mark = ',')
pop <- read_excel('assets/data/sao-paulo/pop.xls', skip = 1, sheet = 2) %>%
  clean_names() %>%
  filter(!is.na(cod_munic)) %>%
  mutate(pop = parse_number(`populacao_estimada`, locale = loc),
         cd_gcmun = paste0(cod_uf, cod_munic)) %>%
