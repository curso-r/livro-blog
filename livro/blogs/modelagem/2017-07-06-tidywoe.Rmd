...
tea_mini <- tea %>% dplyr::select(breakfast, how, where, price)
```
...
tea_mini %>% add_woe(breakfast) %>% slice(1:4) %>% knitr::kable()
```
...
tea_mini %>% add_woe(breakfast, where:price) %>% slice(1:4) %>% knitr::kable()
```
...
tea_mini %>% woe_dictionary(breakfast) %>% slice(1:4) %>% knitr::kable()
```
...
tea_mini_woe_dic_arrumado <- tea_mini_woe_dic %>% 
  mutate(woe = if_else(explanatory == "p_unknown", 0, woe))
...
# Passe esse dicionário para o add_woe()
tea_mini %>% 
  add_woe(breakfast, .woe_dictionary = tea_mini_woe_dic_arrumado) %>% 
  slice(1:4) %>% 
  knitr::kable()
```
...
tea_mini_woe_dic_arrumado %>%
  ...
  ggplot() +
  geom_bar(aes(x = explanatory, y = woe), stat = "identity", fill = "seagreen") +
